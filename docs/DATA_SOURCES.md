...
Regla T-39 aplicada: EMT `estado-servicio` solo promociona a `UrbanEvent` cuando hay `extra_data.geometry` oficial o coincidencia con un gazetteer interno testeado. Cualquier aviso genérico queda en staging, y un evento open data existente a menos de 120 m bloquea la promoción.
