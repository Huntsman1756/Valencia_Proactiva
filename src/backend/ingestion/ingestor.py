...
from ingestion.normalizer import Normalizer, clean_public_text
...
class Ingestor:
    ...
    def _refresh_existing_event(
        ...
        next_title_clean = clean_public_text(next_title) if isinstance(next_title, str) else None
        current_title = event.title.strip() if isinstance(event.title, str) else ""
        ...
        should_refresh_title = (
            ...
            and (
                not event.title
                or event.title.casefold() == "sin titulo"
                or clean_public_text(current_title) != current_title
            )
        )
