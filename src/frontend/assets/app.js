...
const STATIC_ASSET_VERSION = "20260529-public-data-copy";
...
const SOURCE_CATALOG = [
  {
    ...
    groupKey: "sourceGroupOfficial",
    linkKey: "openOfficialSourceLink",
  ...
  },
  {
    ...
    groupKey: "sourceGroupComplementary",
    linkKey: "openSupportSourceLink",
  ...
  },
  {
    ...
    groupKey: "sourceGroupDerived",
    linkKey: "openDerivedDataLink",
    url: "https://vlcproactiva.es/exports/impact_zones.geojson",
  },
];
...
const MEMORY_URL = "https://vlcproactiva.es/docs/MEMORIA.md";
const DATA_SCHEMA_URL = "https://vlcproactiva.es/docs/data-schema.md";
const EXPORT_URLS = {
  impactZones: "https://vlcproactiva.es/exports/impact_zones.geojson",
  mitigationActions: "https://vlcproactiva.es/exports/mitigation_actions.csv",
  feedbackAggregated: "https://vlcproactiva.es/exports/feedback_aggregated.csv",
  latestEvents: "https://vlcproactiva.es/exports/latest_events.json",
  dataHealth: "https://vlcproactiva.es/exports/data_health.json",
};
...
const STREET_INDEX_URL = "./data/valencia-streets.json";
const VALENCIA_STREET_BBOX = [-0.55, 39.25, -0.2, 39.65];
...
const streetSearchResults = document.querySelector("#streetSearchResults");
const withAlternativeFilter = document.querySelector("#withAlternativeFilter");
const eventCountBadge = document.querySelector("#eventCountBadge");
...
const legendTrafficItem = document.querySelector("#legendTrafficItem");
...
const detailCompactSummary = document.querySelector("#detailCompactSummary");
...
async function loadMessages() {
  try {
    state.messages = await fetchJson(`./i18n/${state.language}.json?v=${STATIC_ASSET_VERSION}`);
    document.documentElement.lang = state.messages.lang || state.language;
  } catch (error) {
    state.language = "es";
    state.messages = await fetchJson(`./i18n/es.json?v=${STATIC_ASSET_VERSION}`);
...
async function loadStreetIndex() {
  try {
    const data = await fetchJson(STREET_INDEX_URL);
    state.streetIndex = Array.isArray(data.streets)
      ? data.streets.filter(isValidValenciaStreet).map((street) => ({
        ...street,
        search: street.search || normalizeSearch(street.name || ""),
      }))
      : [];
    window.vproStreetIndex = {
      source: data.source,
      count: state.streetIndex.length,
      scope: data.scope,
    };
  } catch (error) {
    console.warn(`Street index unavailable: ${error.message}`);
    state.streetIndex = [];
  }
}

function isValidValenciaStreet(street) {
  if (!street || typeof street !== "object" || !Array.isArray(street.center) || street.center.length < 2) {
    return false;
  }
  const lon = Number(street.center[0]);
  const lat = Number(street.center[1]);
  return Number.isFinite(lon) && Number.isFinite(lat) && isInsideValenciaStreetBounds(lon, lat);
}

function isInsideValenciaStreetBounds(lon, lat) {
  const [minLon, minLat, maxLon, maxLat] = VALENCIA_STREET_BBOX;
...
function setupProfiles() {
  document.querySelectorAll("[data-profile]").forEach((button) => {
    ...
    button.addEventListener("click", (event) => {
      if (event.target.closest(".profile-help")) {
        document.querySelectorAll("[data-profile]").forEach((item) => {
          if (item !== button) {
            item.removeAttribute("data-help-open");
          }
        });
        const open = button.dataset.helpOpen === "true";
        button.dataset.helpOpen = String(!open);
        return;
      }
      ...
      clearSearchAndStreetSelection({ refresh: true, refocusEvent: true });
      document.querySelectorAll("[data-profile]").forEach((item) => {
    ...
    button.addEventListener("blur", () => {
      button.removeAttribute("data-help-open");
    });
  });
  document.addEventListener("click", (event) => {
    if (!event.target.closest("[data-profile]")) {
      document.querySelectorAll("[data-profile]").forEach((button) => button.removeAttribute("data-help-open"));
    }
  });
}
...
function renderProfileButton(button) {
  ...
  const help = profileHelp(profile);
  ...
  button.innerHTML = `
    ...
    <span class="profile-help-popover" role="tooltip">${escapeHtml(help)}</span>
  `;
}
  ...
  document.querySelectorAll("[data-lang]").forEach((button) => {
    ...
    button.addEventListener("click", async () => {
      ...
      if (state.cards.length > 0) {
        ...
        updateOperationalStatus(visibleCards);
...
function setupViews() {
  ...
  document.querySelectorAll("[data-view-target]").forEach((button) => {
    button.addEventListener("click", () => {
      activateView(button.dataset.viewTarget, { updateHash: true, revealEvents: true });
    });
  });
  document.querySelectorAll("[data-focus-target='events']").forEach((button) => {
    button.addEventListener("click", () => {
      activateView("events", { updateHash: true, revealEvents: true });
      eventsPanel?.scrollIntoView({ block: "nearest" });
    });
  });
}
...
function setupDetailPanelToggle() {
  ...
  setDetailCollapsed(savedState === null ? false : savedState === "true", false);
...
function setupOperationalFilters() {
  eventSearch?.addEventListener("input", () => {
    state.searchRaw = eventSearch.value.trim();
    state.searchQuery = normalizeSearch(state.searchRaw);
  ...
  eventSearch?.addEventListener("keydown", (event) => {
    ...
    if (state.visibleCards.length > 0) {
      event.preventDefault();
      selectCard(0, { flyTo: true, expandDetail: true });
      return;
    }
    if (state.streetMatches.length > 0) {
      event.preventDefault();
      selectStreetResult(state.streetMatches[0]);
    }
  });
  withAlternativeFilter?.addEventListener("change", () => {
    state.onlyWithAlternative = Boolean(withAlternativeFilter.checked);
    refreshOperationalView();
  });
}

function refreshOperationalView() {
  state.streetMatches = streetMatchesForQuery(state.searchQuery);
  const cards = filteredCards();
  renderStreetSearchResults();
  renderCards(cards);
  updateMap(cards);
  updateOperationalStatus(cards);
}
...
async function loadDashboard() {
  ...
  try {
    ...
    if (state.events.length === 0) {
      renderEmpty(t("emptyEvents"), t("emptyEventsHint"));
      updateMap([]);
      updateOperationalStatus([]);
      return;
    }
    ...
    renderInfoPanels();
    updateMap(filteredCards());
    updateOperationalStatus(filteredCards());
    if (state.profileRefocusTimer) {
      window.clearTimeout(state.profileRefocusTimer);
      state.profileRefocusTimer = window.setTimeout(() => focusSelectedEventOnMap(), 80);
    }
...
function renderCards(cards) {
  const previousSelectedId = state.visibleCards[state.selectedCardIndex]?.event?.id;
  state.visibleCards = cards;
  eventList.innerHTML = "";
  if (eventCountBadge) {
    eventCountBadge.textContent = String(cards.length);
  }
  updateLocalDeltaNotice(cards);
  renderMapEventTray(cards);
  if (cards.length === 0) {
    renderSelectedEvent(null);
    eventList.innerHTML = emptyStateMarkup(emptyStateForCurrentFilters());
    return;
  }
  cards.forEach((card, index) => eventList.appendChild(createEventCard(card, index)));
  const retainedIndex = cards.findIndex((card) => String(card.event.id) === String(previousSelectedId));
  selectCard(retainedIndex >= 0 ? retainedIndex : 0, { flyTo: false, expandDetail: false });
}
...
function filteredCards() {
  const baseCards = state.cards.filter((card) => eventPassesOperationalFilters(card));
  const query = state.searchQuery;
  if (!query) {
    return baseCards;
  }
  const eventMatches = baseCards.filter((card) => eventMatchesQuery(card, query));
...
function eventPassesOperationalFilters(card) {
  if (state.onlyWithAlternative && !card.alternative) {
    return false;
  }
...
function eventMatchesQuery(card, query) {
  return eventSearchText(card).includes(query);
}
...
function eventSearchText(card) {
  const event = card.event || {};
  const extra = event.extra_data && typeof event.extra_data === "object" ? event.extra_data : {};
  const alternative = card.alternative || {};
  const alternativeExtra = alternative.extra_data && typeof alternative.extra_data === "object" ? alternative.extra_data : {};
  return normalizeSearch([
    ...
    extra.referencia,
    extra.reference,
    displayAlternativeName(alternative, labelForPoi(alternative.poi_type)),
    labelForPoi(alternative.poi_type),
    alternative.source,
    alternative.id,
    alternativeExtra.barrio,
    alternativeExtra.zona,
  ]
    .filter(Boolean)
    .join(" "));
}

function emptyStateForCurrentFilters() {
  if (state.cards.length === 0) {
    return { title: t("emptyEvents"), hint: t("emptyEventsHint") };
  }
  if (state.searchQuery) {
    return { title: t("noSearchResults"), hint: t("noSearchResultsHint") };
  }
  if (state.onlyWithAlternative) {
    return { title: t("noAlternativeFilterResults"), hint: t("filterEmptyHint") };
  }
  return { title: t("emptyEvents"), hint: t("emptyEventsHint") };
}

function emptyStateMarkup(stateText) {
  return `
    <div class="empty-state">
      <strong>${escapeHtml(stateText.title)}</strong>
      ${stateText.hint ? `<span>${escapeHtml(stateText.hint)}</span>` : ""}
    </div>
  `;
}

function streetMatchesForQuery(query) {
  if (!query || query.length < 2 || state.streetIndex.length === 0) {
    return [];
  }
  return state.streetIndex
    .filter((street) => street.search.includes(query))
    .sort((left, right) => streetMatchRank(left, query) - streetMatchRank(right, query)
      || left.name.localeCompare(right.name, state.language === "val" ? "ca" : "es"))
    .slice(0, 6);
}

function streetMatchRank(street, query) {
  if (street.search === query) {
    return 0;
  }
  if (street.search.startsWith(query)) {
    return 1;
  }
  if (street.search.includes(` ${query}`)) {
    return 2;
  }
  return 3;
}

function renderStreetSearchResults() {
  if (!streetSearchResults) {
    return;
  }
  if (!state.searchQuery || state.searchQuery.length < 2) {
    streetSearchResults.hidden = true;
    streetSearchResults.innerHTML = "";
    return;
  }
  const source = t("streetSearchSource");
  if (state.streetMatches.length === 0) {
    streetSearchResults.hidden = false;
    streetSearchResults.innerHTML = `
      <p>${escapeHtml(t("streetSearchTitle"))}</p>
      <span>${escapeHtml(t("streetSearchEmpty"))}</span>
    `;
    return;
  }
  streetSearchResults.hidden = false;
  streetSearchResults.innerHTML = `
    <p>${escapeHtml(t("streetSearchTitle"))}</p>
    <div role="listbox" aria-label="${escapeHtml(t("streetSearchAria"))}">
      ${state.streetMatches.map((street, index) => `
        <button type="button" data-street-result="${index}" role="option">
  ...
  `;
  streetSearchResults.querySelectorAll("[data-street-result]").forEach((button) => {
    button.addEventListener("click", () => {
      const street = state.streetMatches[Number(button.dataset.streetResult)];
      if (street) {
        selectStreetResult(street);
      }
    });
  });
}

function selectStreetResult(street) {
  state.selectedStreet = street;
  updateStreetSearchMap();
  if (state.map && Array.isArray(street.center)) {
    state.map.flyTo({ center: street.center, zoom: 16, essential: false });
    state.mapPopup?.remove();
    state.mapPopup = new maplibregl.Popup({ closeButton: true, closeOnClick: true })
      .setLngLat(street.center)
      .setHTML(`
        <strong>${escapeHtml(street.name)}</strong>
        <p>${escapeHtml(t("streetSearchPopup"))}</p>
      `)
      .addTo(state.map);
  }
}
...
function clearSearchAndStreetSelection(options = {}) {
  state.searchRaw = "";
  state.searchQuery = "";
  state.streetMatches = [];
  if (eventSearch) {
  ...
  if (options.refocusEvent) {
    window.clearTimeout(state.profileRefocusTimer);
    focusSelectedEventOnMap();
    state.profileRefocusTimer = window.setTimeout(() => focusSelectedEventOnMap(), 360);
  }
}

function alternativeCalculatedCount(cards = state.visibleCards) {
  return cards.filter((card) => Boolean(card.alternative)).length;
}

function updateOperationalStatus(cards = state.visibleCards) {
  if (!statusText) {
    return;
  }
  const count = alternativeCalculatedCount(cards);
  const key = count === 1 ? "activeEventsSingular" : "activeEventsPlural";
  statusText.textContent = formatMessage(key, { count });
}
...
function createEventCard(card, index) {
  ...
  const alternativeSummary = alternative
    ? `${alternativeName} · ${distance} · ${places}`
    : t("noAlternative");

  article.innerHTML = `
    <div class="event-header">
      <span class="event-type">${escapeHtml(labelForType(event.type))}</span>
    ...
    <div class="event-alt-summary">
      <span class="event-alt-label">${escapeHtml(t("alternativeShortLabel"))}</span>
      <span>${escapeHtml(alternativeSummary)}</span>
      ${alternativeSource ? `<small>${escapeHtml(alternativeSource)}</small>` : ""}
    </div>
  `;
...
function selectCard(index, options = {}) {
  ...
  refreshSelectedMapDecision();
  if (options.expandDetail !== false) {
    setDetailCollapsed(false);
  }
  if (options.flyTo !== false && state.map && card?.event?.center) {
    focusSelectedEventOnMap({ zoom: 14 });
  }
}

function focusSelectedEventOnMap(options = {}) {
  const card = state.visibleCards[state.selectedCardIndex] || state.visibleCards[0];
  if (!state.map || !card?.event?.center) {
    return;
  }
  state.map.stop?.();
  state.map.flyTo({
    center: card.event.center,
    zoom: options.zoom || 13.4,
...
function renderSelectedEvent(card) {
  ...
  if (!card) {
    selectedEventPanel.innerHTML = emptyStateMarkup({ title: t("selectEventHint"), hint: t("selectEventHintDetail") });
    updateDetailCompactSummary(null);
    return;
  }
  ...
  const eventLocation = formatEventLocation(event);
  const alternativeSummary = alternative
    ? `${distance} \u00b7 ${places} \u00b7 ${labelForPoi(alternative.poi_type)}`
    : t("noAlternative");
  const alternativeReason = profileAlternativeGuidance(card);
  const sourceId = event.source_id || event.id || t("sourceWithoutReference");
  const sourceDetail = formatMessage("detailSourceText", {
    source: event.source ? labelForSource(event.source) : t("notAvailable"),
    reference: sourceId,
    updated: formatUpdated(event.updated_at || event.created_at),
  });
  const affectedText = formatMessage("detailAffectedText", {
    profile: profileLabel(state.profile),
    impact: impact.meters,
  });

  selectedEventPanel.innerHTML = `
    <div class="detail-meta detail-narrative-head">
      <div class="detail-badges">
        <span class="detail-status${severityClass}">${escapeHtml(impact.label)}</span>
        <span class="detail-source-badge">${escapeHtml(t("officialSourceBadge"))}</span>
      </div>
      <h2>${escapeHtml(event.title.toUpperCase())}</h2>
      <p>${escapeHtml(trimText(event.description, 132))}</p>
    </div>
    <div class="detail-story">
      <section class="detail-story-item">
        <h3>${t("detailWhatTitle")}</h3>
        <p>${escapeHtml(labelForType(event.type))} · ${escapeHtml(temporalText)}</p>
      </section>
      <section class="detail-story-item">
        <h3>${t("detailWhereTitle")}</h3>
        <p>${escapeHtml(eventLocation)}</p>
      </section>
      <section class="detail-story-item">
        <h3>${t("detailAffectedTitle")}</h3>
        <p>${escapeHtml(affectedText)}</p>
      </section>
      <section class="detail-story-item detail-decision-block">
        <h3>${t("detailAlternativeTitle")}</h3>
        <p><strong>${escapeHtml(alternativeName || t("noAlternative"))}</strong></p>
        <p>${escapeHtml(alternativeSummary)}</p>
      </section>
      <section class="detail-story-item detail-reason-story">
        <h3>${t("detailReasonTitle")}</h3>
        <p>${escapeHtml(alternativeReason)}</p>
      </section>
      <section class="detail-story-item detail-source-story">
        <h3>${t("detailSourceTitle")}</h3>
        <p>${escapeHtml(sourceDetail)}</p>
        <div class="evidence-actions">
          <a href="${SOURCE_CATALOG[0].url}" target="_blank" rel="noopener">${escapeHtml(t("sourceMunicipalLink"))}</a>
          <a href="${eventPublicUrl(event)}" target="_blank" rel="noopener">${escapeHtml(t("publicFileLink"))}</a>
          <a href="${EXPORT_URLS.latestEvents}" target="_blank" rel="noopener">${escapeHtml(t("exportJsonLink"))}</a>
        </div>
      </section>
    </div>
    <section class="detail-block detail-cta-block">
  ...
  `;
  updateDetailCompactSummary(card);
...
function updateDetailCompactSummary(card) {
  if (!detailCompactSummary) {
    return;
  }
  if (!card) {
    detailCompactSummary.textContent = t("selectEventHint");
    return;
  }
  const impact = impactInfo(card.event);
  const alternativeDistance = card.alternative?.distance_meters
    ? `${Math.round(card.alternative.distance_meters)} m`
    : t("near");
  detailCompactSummary.textContent = formatMessage("detailCompactSummary", {
    location: formatEventLocation(card.event),
    impact: impact.meters,
...
function labelForActionLink(action) {
  if (action?.payload?.template_id === "comercio-ocupacion") {
    return t("municipalReferenceLink");
  }
  return t("openActionLink");
}

function eventPublicUrl(event) {
  return `https://vlcproactiva.es/exports/events/${encodeURIComponent(event?.id || "")}.html`;
}
...
function formatEditorialText(value) {
  const text = cleanDisplayPlace(value || "");
  if (!text || /[a-zà-ÿ]/u.test(text)) {
    return text;
  }
  return text
    .toLocaleLowerCase(state.language === "val" ? "ca-ES" : "es-ES")
    .replace(/(^|[\s/([{¿¡-])(\p{L})/gu, (match, prefix, letter) => `${prefix}${letter.toLocaleUpperCase(state.language === "val" ? "ca-ES" : "es-ES")}`)
    .replace(/^C\/\s*/u, "C/ ")
    .replace(/^Av\.\s*/u, "Av. ")
    .replace(/^Pl\.\s*/u, "Pl. ");
}
...
function renderEmpty(message, hint = "") {
  eventList.innerHTML = emptyStateMarkup({ title: message, hint });
  state.cards = [];
  state.visibleCards = [];
...
function renderSourceTrace() {
  const steps = [
    ["sourceTraceOfficialTitle", "sourceTraceOfficialText"],
    ["sourceTraceNormalizeTitle", "sourceTraceNormalizeText"],
    ["sourceTraceGeometryTitle", "sourceTraceGeometryText"],
    ["sourceTraceImpactTitle", "sourceTraceImpactText"],
    ["sourceTraceInterpretTitle", "sourceTraceInterpretText"],
    ["sourceTraceDerivedTitle", "sourceTraceDerivedText"],
  ];
  return `
    <section class="source-trace" aria-label="${escapeHtml(t("sourceTraceTitle"))}">
      <div>
        <p class="eyebrow">${escapeHtml(t("sourceTraceEyebrow"))}</p>
        <h2>${escapeHtml(t("sourceTraceTitle"))}</h2>
        <p>${escapeHtml(t("sourceTraceText"))}</p>
      </div>
      <ol>
...
function renderSourceMetrics() {
  const metrics = [
    [SOURCE_CATALOG.find((source) => source.id === "open_data_core")?.datasetCount || 14, t("sourceMetricDatasets")],
    [SOURCE_CATALOG.find((source) => source.id === "vpro_feedback")?.datasetCount || 3, t("sourceMetricDerived")],
    [publicFeedCount(), t("sourceMetricRecent")],
    [operationalEventCount(), t("sourceMetricProcessed")],
  ];
  return `
    <section class="source-metrics" aria-label="${escapeHtml(t("sourceMetricsTitle"))}">
...
function renderSourceActions() {
  const actions = [
    ["sourceActionOfficialTitle", "sourceActionOfficialText", SOURCE_CATALOG[0].url],
    ["sourceActionImpactTitle", "sourceActionImpactText", EXPORT_URLS.impactZones],
    ["sourceActionSchemaTitle", "sourceActionSchemaText", DATA_SCHEMA_URL],
    ["sourceActionCodeTitle", "sourceActionCodeText", GITHUB_URL],
  ];
  return `
    <section class="source-actions" aria-label="${escapeHtml(t("sourceActionsTitle"))}">
      ${actions.map(([titleKey, textKey, url]) => `
...
function renderSourceCatalog() {
  return `
    <div class="source-list source-list-redesigned">
      ${SOURCE_CATALOG.map((source) => renderSourceBlock(source)).join("")}
    </div>
  `;
}

function renderSourceBlock(source) {
  const datasets = sourceDatasets(source.id);
  const countKey = source.countKey || "datasetCount";
  const linkKey = source.linkKey || "openSourceLink";
  return `
    <article class="source-row source-row-${escapeHtml(source.id)}">
      <header class="source-row-head">
        <div>
          <p class="source-group">${escapeHtml(t(source.groupKey || source.kindKey))}</p>
          <strong>${escapeHtml(t(`sourceName_${source.id}`))}</strong>
          <span>${escapeHtml(t(`sourceDescription_${source.id}`))}</span>
        </div>
        <a href="${source.url}" target="_blank" rel="noopener">${escapeHtml(t(linkKey))}</a>
      </header>
      <div class="source-row-meta">
        <small>${escapeHtml(t(source.kindKey))}</small>
        <small>${escapeHtml(formatMessage(countKey, { count: source.datasetCount }))}</small>
      </div>
...
function renderMethodTrace() {
  const steps = [
    ["methodTraceInputTitle", "methodTraceInputText"],
    ["methodTraceEstimateTitle", "methodTraceEstimateText"],
    ["methodTraceRecommendTitle", "methodTraceRecommendText"],
    ["methodTracePublishTitle", "methodTracePublishText"],
  ];
  return `
    <section class="method-chain" aria-label="${escapeHtml(t("methodTraceTitle"))}">
      <div>
        <p class="eyebrow">${escapeHtml(t("methodTraceEyebrow"))}</p>
        <h2>${escapeHtml(t("methodTraceTitle"))}</h2>
        <p>${escapeHtml(t("methodTraceText"))}</p>
      </div>
      <ol>
        ${steps.map(([titleKey, textKey]) => `
          <li>
...
function renderMethodSteps() {
  return `
    <section class="info-section method-steps">
      <h3>${escapeHtml(t("methodStepsTitle"))}</h3>
      <ol class="method-list">
        ${Array.from({ length: 5 }, (_, index) => {
          const number = index + 1;
          return `
            <li>
              <strong>${escapeHtml(t(`methodStepTitle${number}`))}</strong>
              <span>${escapeHtml(t(`methodStep${number}`))}</span>
            </li>
          `;
        }).join("")}
      </ol>
...
function renderMethodBounds() {
  const official = ["methodOfficial1", "methodOfficial2", "methodOfficial3"];
  const calculated = ["methodCalc1", "methodCalc2", "methodCalc3"];
  const notReplaced = ["methodNoReplaceSummary", "methodNoReplace1", "methodNoReplace2", "methodNoReplace3"];
  return `
    <section class="method-bounds" aria-label="${escapeHtml(t("methodBoundsTitle"))}">
      <div class="method-section-heading">
        <p class="eyebrow">${escapeHtml(t("methodBoundsEyebrow"))}</p>
        <h2>${escapeHtml(t("methodBoundsTitle"))}</h2>
        <p>${escapeHtml(t("methodBoundsIntro"))}</p>
      </div>
      <div class="method-bounds-grid">
        <article>
          <h3>${escapeHtml(t("methodOfficialTitle"))}</h3>
          <ul>
            ${official.map((key) => `<li>${escapeHtml(t(key))}</li>`).join("")}
          </ul>
        </article>
        <article>
          <h3>${escapeHtml(t("methodCalculatesTitle"))}</h3>
          <ul>
            ${calculated.map((key) => `<li>${escapeHtml(t(key))}</li>`).join("")}
          </ul>
        </article>
        <article>
          <h3>${escapeHtml(t("methodDoesNotReplaceTitle"))}</h3>
          <ul>
            ${notReplaced.map((key) => `<li>${escapeHtml(t(key))}</li>`).join("")}
          </ul>
        </article>
      </div>
    </section>
...
function renderMethodAudit() {
  const resources = [
    ["methodAuditEventsTitle", "methodAuditEventsText", EXPORT_URLS.latestEvents],
    ["methodAuditImpactTitle", "methodAuditImpactText", EXPORT_URLS.impactZones],
    ["methodAuditActionsTitle", "methodAuditActionsText", EXPORT_URLS.mitigationActions],
    ["methodAuditSchemaTitle", "methodAuditSchemaText", DATA_SCHEMA_URL],
  ];
  return `
    <section class="method-audit" aria-label="${escapeHtml(t("methodAuditTitle"))}">
      <div>
        <p class="eyebrow">${escapeHtml(t("methodAuditEyebrow"))}</p>
        <h2>${escapeHtml(t("methodAuditTitle"))}</h2>
        <p>${escapeHtml(t("methodAuditIntro"))}</p>
      </div>
      <div class="method-audit-grid">
        ${resources.map(([titleKey, textKey, url]) => `
          <a href="${url}" target="_blank" rel="noopener">
            <strong>${escapeHtml(t(titleKey))}</strong>
            <span>${escapeHtml(t(textKey))}</span>
          </a>
...
function renderMethodExample() {
  const card = state.visibleCards[0] || state.cards[0] || null;
  if (!card) {
    return `
      <section class="method-example info-section">
        <p class="eyebrow">${escapeHtml(t("methodExampleEyebrow"))}</p>
        <h3>${escapeHtml(t("methodExampleTitle"))}</h3>
        <p class="info-copy">${escapeHtml(t("methodExampleUnavailable"))}</p>
      </section>
    `;
  }
  const { event, alternative } = card;
  const impact = impactInfo(event);
  const alternativeName = formatEditorialText(displayAlternativeName(alternative, labelForPoi(alternative?.poi_type))) || t("noAlternative");
  const eventLocation = formatEditorialText(formatEventLocation(event));
  const rows = [
    [t("methodExampleLocation"), eventLocation],
    [t("methodExampleSource"), labelForSource(event.source)],
    [t("methodExampleImpact"), formatMessage("affectedAreaValue", { meters: impact.meters })],
    [t("methodExampleProfile"), profileLabel(state.profile)],
    [t("methodExampleAlternative"), alternative ? `${alternativeName} · ${Math.round(alternative.distance_meters || 0)} m` : t("noAlternative")],
    [t("methodExampleLimitation"), t("methodExampleLimitText")],
  ];
  return `
    <section class="method-example info-section">
      <div>
        <p class="eyebrow">${escapeHtml(t("methodExampleEyebrow"))}</p>
        <h3>${escapeHtml(t("methodExampleTitle"))}</h3>
        <p class="info-copy">${escapeHtml(t("methodExampleIntro"))}</p>
      </div>
      <dl>
...
function renderInfoStory() {
  const audiences = ["infoAudiencePmr", "infoAudienceCommerce", "infoAudienceNeighbors", "infoAudienceTransport", "infoAudienceBike"];
  const evidence = ["infoEvidenceAlternatives", "infoEvidenceRecent", "infoEvidenceProcessed", "infoEvidenceDownloads"];
  return `
    <section class="info-story">
      <article class="info-story-main">
        <p class="eyebrow">${escapeHtml(t("infoProblemEyebrow"))}</p>
        <h2>${escapeHtml(t("infoProblemTitle"))}</h2>
        <p>${escapeHtml(t("infoProblemText"))}</p>
        <h3>${escapeHtml(t("infoSolutionTitle"))}</h3>
        <p>${escapeHtml(t("infoSolutionText"))}</p>
      </article>
      <aside class="info-story-side">
        <h3>${escapeHtml(t("infoAudienceTitle"))}</h3>
        <ul>
          ${audiences.map((key) => `<li>${escapeHtml(t(key))}</li>`).join("")}
        </ul>
        <h3>${escapeHtml(t("infoEvidenceProducesTitle"))}</h3>
        <ul>
          ${evidence.map((key) => `<li>${escapeHtml(t(key))}</li>`).join("")}
        </ul>
      </aside>
    </section>
  `;
...
function renderAuditPackage() {
  const resources = [
    ["auditCodeTitle", "auditCodeText", GITHUB_URL],
    ["auditReleaseTitle", "auditReleaseText", RELEASE_URL],
    ["auditOfficialTitle", "auditOfficialText", SOURCE_CATALOG[0].url],
    ["auditImpactTitle", "auditImpactText", EXPORT_URLS.impactZones],
    ["auditActionsTitle", "auditActionsText", EXPORT_URLS.mitigationActions],
    ["auditFeedbackTitle", "auditFeedbackText", EXPORT_URLS.feedbackAggregated],
    ["auditLatestTitle", "auditLatestText", EXPORT_URLS.latestEvents],
    ["auditHealthTitle", "auditHealthText", EXPORT_URLS.dataHealth],
    ["auditMemoryTitle", "auditMemoryText", MEMORY_URL],
    ["auditSchemaTitle", "auditSchemaText", DATA_SCHEMA_URL],
  ];
  return `
    <section class="audit-package" aria-label="${escapeHtml(t("auditPackageTitle"))}">
      <div class="audit-package-copy">
        <p class="eyebrow">${escapeHtml(t("auditPackageEyebrow"))}</p>
        <h2>${escapeHtml(t("auditPackageTitle"))}</h2>
        <p>${escapeHtml(t("auditPackageText"))}</p>
      </div>
      <div class="audit-resource-list">
        ${resources.map(([titleKey, textKey, url]) => `
          <a href="${url}" target="_blank" rel="noopener">
            <strong>${escapeHtml(t(titleKey))}</strong>
            <span>${escapeHtml(t(textKey))}</span>
...
function renderInfoPanels() {
  sourcesPanel.innerHTML = `
    <div class="panel-heading info-hero">
      <p class="eyebrow">${t("sourcesEyebrow")}</p>
      <h1>${t("sourcesTitle")}</h1>
      <p>${t("sourcesIntro")}</p>
      <p class="info-value-chain">${t("sourcesTraceChain")}</p>
    </div>
    ${renderSourceMetrics()}
    ${renderSourceActions()}
    ${renderSourceTrace()}
  ...
  methodologyPanel.innerHTML = `
    <div class="panel-heading info-hero methodology-hero">
      <p class="eyebrow">${t("methodologyEyebrow")}</p>
      <h1>${t("methodologyTitle")}</h1>
      <p>${t("methodologyIntro")}</p>
      <p class="info-value-chain">${t("methodologyChain")}</p>
    </div>
    ${renderMethodTrace()}
    ${renderMethodSteps()}
    ${renderMethodBounds()}
    ${renderMethodAudit()}
    ${renderMethodExample()}
    <details class="info-accordion method-symbols">
    ...
    <p class="info-copy method-closing">${t("methodologyNote")}</p>
  `;
  ...
  additionalInfoPanel.innerHTML = `
    <div class="panel-heading info-hero">
      ...
      <h1>${t("infoTitle")}</h1>
      <p>${t("infoIntro")}</p>
      <p class="info-value-chain">${t("infoValueChain")}</p>
    </div>
    ${renderInfoStory()}
    ${renderAuditPackage()}
    ${renderCivicUseCase()}
    ${renderContestProof()}
    <section class="info-section info-evidence">
      <h3>${t("visibleDataTitle")}</h3>
      ${renderInfoEvidence()}
      <p class="info-copy">${t("visibleDataText")}</p>
    </section>
    ...
    <details class="info-accordion faq-accordion">
...
function renderCivicUseCase() {
  const card = state.visibleCards[0] || state.cards[0] || null;
  if (!card) {
    return `
      <section class="civic-proof">
        <div>
          <p class="eyebrow">${escapeHtml(t("traceableCaseEyebrow"))}</p>
          <h2>${escapeHtml(t("traceableCaseTitle"))}</h2>
          <p>${escapeHtml(t("traceableCaseUnavailable"))}</p>
        </div>
      </section>
    `;
  }
  ...
  const impact = impactInfo(event);
  const eventLocation = formatEditorialText(formatEventLocation(event));
  const alternativeName = formatEditorialText(displayAlternativeName(alternative, labelForPoi(alternative?.poi_type)));
  const alternativeText = alternative
    ? `${alternativeName} - ${Math.round(alternative.distance_meters || 0)} m`
    : t("noAlternative");
  const sourceId = event.source_id || event.id || t("sourceWithoutReference");
  const rows = [
    [t("traceableCaseLocation"), eventLocation],
    [t("traceableCaseSource"), `${labelForSource(event.source)} - ${sourceId}`],
    [t("traceableCaseImpact"), formatMessage("affectedAreaValue", { meters: impact.meters })],
    [t("traceableCaseProfile"), profileLabel(state.profile)],
    [t("traceableCaseAlternative"), alternativeText],
    [t("traceableCaseEvidence"), t("traceableCaseEvidenceText")],
  ];
  return `
    <section class="civic-proof">
      <div>
        <p class="eyebrow">${escapeHtml(t("traceableCaseEyebrow"))}</p>
        <h2>${escapeHtml(t("traceableCaseTitle"))}</h2>
        <p>${escapeHtml(t("traceableCaseText"))}</p>
        <p class="case-proof-place">
          <span>${escapeHtml(t("traceableCaseLocation"))}</span>
          <strong>${escapeHtml(eventLocation)}</strong>
        </p>
      </div>
      <dl class="case-proof-list">
        ${rows.map(([label, value]) => `
          <div>
            <dt>${escapeHtml(label)}</dt>
            <dd>${escapeHtml(value)}</dd>
...
function renderInfoEvidence() {
  const metrics = [
    [visibleEventCount(), t("evidenceVisibleLabel")],
    [publicFeedCount(), t("evidenceFeedLabel")],
    [operationalEventCount(), t("evidenceTotalLabel")],
  ];
  return `
    <dl class="info-evidence-grid">
      ${metrics.map(([value, label]) => `
        <div>
          <dt>${escapeHtml(String(value))}</dt>
...
function updateMap(cards) {
  ...
  if (typeof state.map.isStyleLoaded === "function" && !state.map.isStyleLoaded()) {
    window.setTimeout(() => updateMap(cards), 120);
    return;
  }
  ...
  if (legendTrafficItem) {
    legendTrafficItem.hidden = relevantTrafficEvents.length === 0;
  }
...
function ensureMapLayers() {
  ...
  state.map.addSource("selected-decision-line", { type: "geojson", data: featureCollection([]) });
  state.map.addSource("selected-alternative-point", { type: "geojson", data: featureCollection([]) });
  state.map.addSource("street-search-point", { type: "geojson", data: featureCollection([]) });
  ...
  state.map.addLayer({
    id: "selected-decision-line",
    type: "line",
    source: "selected-decision-line",
    paint: {
      "line-color": "#0d3b66",
      "line-width": ["interpolate", ["linear"], ["zoom"], 11, 1.4, 15, 2.4],
      "line-opacity": 0.72,
      "line-dasharray": [2, 2],
    },
  });
  state.map.addLayer({
    ...
    layout: {
      ...
      "icon-size": ["interpolate", ["linear"], ["zoom"], 11, 0.58, 15, 0.82],
      "icon-anchor": "bottom",
      "icon-allow-overlap": true,
      "icon-ignore-placement": true,
    },
  ...
  state.map.addLayer({
    id: "selected-alternative-halo",
    type: "circle",
    source: "selected-alternative-point",
    paint: {
      "circle-radius": ["interpolate", ["linear"], ["zoom"], 11, 13, 15, 20],
      "circle-color": "rgba(13, 59, 102, 0.16)",
      "circle-stroke-color": "#0d3b66",
      "circle-stroke-width": ["interpolate", ["linear"], ["zoom"], 11, 2, 15, 3],
    },
  });
  state.map.addLayer({
    id: "selected-alternative-point",
    type: "symbol",
    source: "selected-alternative-point",
    layout: {
      "icon-image": "vpro-alt-selected-marker",
      "icon-size": ["interpolate", ["linear"], ["zoom"], 11, 0.9, 15, 1.22],
      "icon-anchor": "bottom",
      "icon-allow-overlap": true,
      "icon-ignore-placement": true,
    },
  ...
  state.map.addLayer({
    id: "street-search-point",
    type: "circle",
    source: "street-search-point",
    paint: {
...
function ensureMapMarkerImages() {
  [
    ...
    ["vpro-alt-marker", "#546579", "A"],
    ["vpro-alt-selected-marker", "#0d3b66", "A"],
...
function setupMapInteractions() {
  ...
  [
    ...
    ["selected-alternative-point", popupForAlternativePoint],
    ...
    state.map.on("click", layerId, (event) => {
      ...
      if (layerId === "event-points") {
        const cardIndex = state.visibleCards.findIndex((card) => String(card.event.id) === String(feature.properties?.id));
        if (cardIndex >= 0) {
          selectCard(cardIndex, { flyTo: false, expandDetail: true });
        }
      }
      state.mapPopup?.remove();
      state.mapPopup = new maplibregl.Popup({ closeButton: true, closeOnClick: true })
...
function popupForAlternativePoint(properties) {
  ...
  const label = properties.selected === true || properties.selected === "true"
    ? t("legendSelectedAlternative")
    : t("legendAlternative");
  return popupHtml(properties.name || labelForPoi(properties.poi_type), [
    `${label} - ${labelForPoi(properties.poi_type)}`,
...
function updateStreetSearchMap() {
  if (!state.map?.getSource("street-search-point")) {
    return;
  }
  const street = state.selectedStreet;
  if (!street || !Array.isArray(street.center)) {
    setGeoJsonSource("street-search-point", featureCollection([]));
    return;
  }
  setGeoJsonSource("street-search-point", featureCollection([{
    type: "Feature",
    geometry: { type: "Point", coordinates: street.center },
    properties: {
      id: street.id,
      name: street.name,
      source: "Ejes lineales de las calles",
    },
  }]));
}

function refreshSelectedMapDecision() {
  if (!state.map) {
    return;
  }
  const card = state.visibleCards[state.selectedCardIndex];
  setGeoJsonSource("selected-alternative-point", featureCollection([
    alternativeFeature(card?.alternative, { selected: true }),
  ].filter(Boolean)));
  setGeoJsonSource("selected-decision-line", featureCollection([
    decisionLineFeature(card),
  ].filter(Boolean)));
}
...
function alternativeFeature(alternative, options = {}) {
...
function decisionLineFeature(card) {
  const eventCenter = Array.isArray(card?.event?.center) ? card.event.center : null;
  const alternativeCenter = pointCoordinatesFromGeometry(card?.alternative?.geometry);
  if (!eventCenter || !alternativeCenter) {
    return null;
  }
  return {
    type: "Feature",
    geometry: { type: "LineString", coordinates: [eventCenter, alternativeCenter] },
    properties: {
      selected: true,
      event_id: card.event.id,
      alternative_id: card.alternative?.id,
    },
  };
}

function pointCoordinatesFromGeometry(geometry) {
...
function profileHelp(profile) {
  return state.messages.profileHelp?.[profile] || profileDescription(profile);
}
